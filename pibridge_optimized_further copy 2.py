...
while True:
    if rs485_ioprotocol:
        ...
        bytes_to_read = 0
        iop_address = iop_cmd = iop_len = -1
        while True:
            if uart_pibridge.any():
                msg += uart_pibridge.read(1)

                if bytes_to_read == 0 and len(msg) == 2: 
                    # received address, command and data length
                    # 0x3f -> address_mask
                    iop_address = msg[0] & MASK_ADDRESS
                    iop_cmd = (msg[1] & MASK_CMD) >> 5
                    iop_len = msg[1] & MASK_LEN

                    bytes_to_read = iop_len + IOPROTOCOL_CRC_LENGTH

                    # print(iop_address, iop_cmd, iop_len)
                    # print_hex(msg)

...
# ModGate commands
# see ModGateRS485.h type   ERs485Command
CMD_PING = 0x0002
...
IOPROTOCOL_CRC_LENGTH = 1

# PiGate commands (IO protocol)
...
sn = 12345
# see common_define.h (KUNBUS_FW_DESCR_TYP_PI_DO_16 et al.)
# 96 = DIO
# 97 = DI
# 98 = DO
module_type = 98
...
input_length = 70
output_length = 18
# type MODGATE_feature_RS485DataExchange
# 0x0001 = Ethernet data-exchange
# 0x0002 = RS485 data-exchange 
...
OUTPUT_PINS = [PIN_OUTPUT_1, PIN_OUTPUT_2, PIN_OUTPUT_3, PIN_OUTPUT_4, PIN_OUTPUT_5, PIN_OUTPUT_6, PIN_OUTPUT_7, PIN_OUTPUT_8]
INPUT_PINS = []
    ...
    if rs485_ioprotocol:
        ...
        if uart_pibridge.any():
            ...
            if msg[-1] == crc:
                ...
                if len(msg) < iop_len + IOPROTOCOL_HEADER_LENGTH + IOPROTOCOL_CRC_LENGTH:
                    # telegram not complete
                    # TODO: can we check iop_cmd and iop_len before CRC calculation?
                    continue
                elif len(msg) > iop_len + IOPROTOCOL_HEADER_LENGTH + IOPROTOCOL_CRC_LENGTH:
                    msg=bytes()
                    print("gnarf discard 2 (len >)")
                    continue

                data = msg[IOPROTOCOL_HEADER_LENGTH:IOPROTOCOL_HEADER_LENGTH+iop_len]

                # TODO: parse commands which affect all modules (like to go back to gateway procotol)
                # TODO: Do we need this? Or was this functionality never implemented?

                if not msg[0] == address:
                    # skip telegram which is not meant for this module
                    print("skip")
                    msg = bytes()
                    continue
                ...
                msg = bytes()
            elif len(msg) > IOPROTOCOL_HEADER_LENGTH + IOPROTOCOL_MAXDATA_LENGTH + IOPROTOCOL_CRC_LENGTH:
                # discard msg buffer if buffer if bigger than maximum message length
                print("discard rx buffer (max length)")
                print_hex(msg)
                msg = bytes()
    else:
        # configuration protocol
        if stage == 0 and sniff_2.value() == True:
            # sniff 2 is driven high by RevPi or other module -> start configuration sequence
            print("stage 0")
            configured = False
            stage = 1
            led.value(False)
            ...
            # TODO: Do we need this timeout check really?
            if delta > 200:
                # timeout
                stage=0
                if uart_pibridge.any():
                ...
                msg=bytes()
                reset_sniff2()
                print(f"timeout after {delta} ms")
                continue
                ...
                if uart_pibridge.any():
                    ...
                    if msg[-1] == crc:
                        ...
                        if t.command == CMD_GET_DEVICE_INFO:
                            ...
                            # 0=PC, 1=left mGate, 2=right mGate
                            t.destination = 0
            ...
            # wait for cyclic communication start
            # TODO: does the current implementation work with multiple modules?
        else:
            if uart_pibridge.any():
                ...
                if msg[-1] == crc:
                    if msg == bytearray([0x7f, 0x00, 0x7f]):
                        # This is a IOProtocol Telegram Type 2 (IoProtocol.h sHeaderTyp2)
                        # first byte: 0x7f = 0111 1111
                        #   bit 0-5:    command     0x3f=IOP_TYP2_CMD_GOTO_GATE_PROTOCOL
                        #   bit 6:      telegram    0=telegram1, 1=telegram2
                        #   bit 7:      msgtype     0=request, 1=response
                        # second byte: 0x00 = 0000 0000
                        #   bit 0-4:    length
                        #   bit 5-7:    data part1

                        print("switch to modgate procotol is requested")
                        rs485_ioprotocol = False
